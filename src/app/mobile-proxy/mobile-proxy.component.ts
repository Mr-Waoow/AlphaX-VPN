...
export class MobileProxyComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.dataService.getMajorServices().subscribe({
      ...
      error: (error) =>
        console.error('Error fetching major services data', error),
    });
    ...
    this.dataService.getPrivateRentFeatures().subscribe({
      ...
      error: (error) =>
        console.error('Error fetching private rent features data', error),
    });
    this.dataService.getMobileProxiesProps().subscribe({
      ...
      error: (error) =>
        console.error('Error fetching mobile proxies props data', error),
    });
    this.dataService.getFrequentlyAskes().subscribe({
      ...
      error: (error) =>
        console.error('Error fetching frequently askes data', error),
    });
  }
